...
describe('draftReducer', () => {
  ...
  it('sets remainingRerolls, clamped between 0 and the pool from the rerolls trait', () => {
    const start = draftWith({ traits: { ...emptyDraft().traits, rerolls: 2 } }) // pool = 3
    expect(reduce(start, { type: 'setRemainingRerolls', value: 2 }).remainingRerolls).toBe(2)
    expect(reduce(start, { type: 'setRemainingRerolls', value: -1 }).remainingRerolls).toBe(0)
    expect(reduce(start, { type: 'setRemainingRerolls', value: 9 }).remainingRerolls).toBe(3)
  })

  it('shrinks a saved remainingRerolls if lowering the rerolls trait drops it below the pool', () => {
    const start = draftWith({
      traits: { ...emptyDraft().traits, rerolls: 2 }, // pool = 3
      remainingRerolls: 3,
    })
    const d = reduce(start, { type: 'setTrait', key: 'rerolls', value: 1 }) // pool = 1
    expect(d.remainingRerolls).toBe(1)
  })
